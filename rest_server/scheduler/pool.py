...
class WorkersPool:
  ...
  async def schedule_task(self, task: WorkerTask) -> any:
    worker = None
    try:
      worker = await self._take_worker()
      started = time.perf_counter()
      result = await task.execute_on(worker)
      waited = round((time.perf_counter() - started) * 1000)
      server_logging.info(f"After {waited}ms got results from worker {worker.short_str}")
      return result
    finally:
      self.__free_worker(worker) if worker else None
      
  async def _take_worker(self) -> Worker:
    worker = self.__find_free_worker()
    if worker:
      server_logging.info(f"Using free worker {worker.full_str}")
      worker.set_busy()
      return worker
  ...
  def __free_worker(self, worker: Worker):
    if not self._queue.empty():
      server_logging.info(f"Passing worker {worker.short_str} to the next waiting request")
  ...
  def __find_free_worker(self) -> Worker | None:
...
class RoundRobinWorkersPool(WorkersPool):
  async def _take_worker(self) -> Worker:
    worker = await super()._take_worker()
...
class WorkerPromise:
  ...
  async def wait_worker(self) -> Worker:
    started = time.perf_counter()
    server_logging.info(f"Waiting for a free worker...")

    await self.__event.wait()

    waited = time.perf_counter() - started
    server_logging.info(f"After {waited:.2f}s got free worker {self.__worker.full_str}")
